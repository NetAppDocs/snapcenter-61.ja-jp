...
summary: ホストを追加してプラグイン パッケージをインストールする前に、すべての要件を満たしておく必要があります。Windows、Linux、AIX 環境でサポートされる IBM Db2 用のSnapCenterプラグイン。 
---
...
SnapCenterプラグイン for IBM Db2 で補足コマンドを実行するには、そのコマンドを _allowed_commands.config_ ファイルに含める必要があります。
